(function () {
  ...
  wb.BluetoothRemoteGATTServer = function (webBluetoothDevice) {
    if (webBluetoothDevice === undefined) {
      throw new Error(
        "Attempt to create BluetoothRemoteGATTServer with no device"
      );
    }
  ...
  wb.BluetoothRemoteGATTServer.prototype = {
    connect: function () {
      ...
      return this.sendMessage("connectGATT", { callbackID: tid }).then(
        function () {
          self.connected = true;
    ...
    disconnect: function () {
      this.connectionTransactionIDs.forEach((tid) =>
        wb.native.cancelTransaction(tid)
      );
      this.connectionTransactionIDs = [];
      if (!this.connected) {
        return;
      ...
      this.device.dispatchEvent(
        new wb.BluetoothEvent("gattserverdisconnected", this.device)
      );
    ...
    getPrimaryServices: async function (uuid) {
      const data = {
        data: uuid
          ? { serviceUUID: window.BluetoothUUID.getService(uuid) }
          : {},
      };
      const serviceUUIDs = await this.sendMessage("getPrimaryServices", data);
      console.log(serviceUUIDs);
      return serviceUUIDs.map(
        (uuidInner) =>
          new wb.BluetoothRemoteGATTService(
            this.device,
